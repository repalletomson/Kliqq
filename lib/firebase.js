...
// Helper function to get user from context or auth
export const getCurrentUserFromContext = (userFromContext) => {
  if (userFromContext) {
    return {
      uid: userFromContext.uid,
      email: userFromContext.email || '',
      displayName: userFromContext.displayName || userFromContext.full_name || '',
      groups: userFromContext.groups || [],
    };
  }
...
export const joinGroup = async (userId, groupId) => {
  try {
    ...
    if (!userDoc.exists()) {
      // Create the user document with the group
